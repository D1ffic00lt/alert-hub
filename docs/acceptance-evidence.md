...
For the current implementation, the latest local backend run completed **174 tests**
with **82.1% branch coverage** against a **75% CI floor**; Ruff, strict mypy, migrations, and the
OpenAPI contract also passed locally. Frontend lint/types, 15 unit tests, the production build,
four Playwright paths, the two-image Docker matrix, and the controlled three-node gate also passed.
...
| Area                                | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Level                  |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| Backend behavior                    | Ruff, strict mypy, a 174-test pytest suite with 82.1% measured branch coverage and a 75% gate, clean/previous-schema migration exercise, and generated OpenAPI contract comparison                                                                                                                                                                                                                                                                                                                               | Automated              |
...
| Frontend and PWA                    | Formatting, ESLint, TypeScript, 15 component/service-worker tests, production build, and four Playwright primary paths passed locally for this working tree. No automated browser result substitutes for real iPhone Home Screen behavior.                                                                                                                                                                                                                                                                       | Automated              |
| Supply chain                        | Secret scan, dependency integrity/audits, pinned GitHub Actions, least-privilege PR policy, candidate/release image policy, SBOM, provenance, and attestation workflow checks                                                                                                                                                                                                                                                                                                                                    | Automated              |
| Public proxy boundary               | Static policy tests assert that the public Nginx/Caddy examples deny `/internal/*`, `/metrics`, `/health/deep`, `/api/docs*`, `/api/redoc*`, and `/api/openapi.json`; real proxy validation and exposure scans remain external                                                                                                                                                                                                                                                                                   | Automated              |
...
| Production network exposure        | External scan showing only intended `443` (and restricted `22` where required), plus peer-only reachability over the actual private interface                                                                                                                                                         | Pending external acceptance  |
...
| Release publication                | Successful protected `vX.Y.Z` workflow, GHCR digest, SBOM, provenance/attestation verification, and GitHub Release link                                                                                                                                                                               | Pending first release        |
| GitHub UI deployment               | Configured protected environments and repository-scoped runners; ordered RU → NL → DE rollout with per-node manifest and health evidence                                                                                                                                                              | Pending infrastructure setup |
...
- Outbound destinations are resolved and validated immediately before bounded requests, but
  the application does not pin the validated IP through the operating-system connect call.
  An egress firewall remains required to close the DNS-rebinding time-of-check/time-of-use gap.
