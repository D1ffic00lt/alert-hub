...
5. a repository-scoped GitHub runner asks a root-owned local wrapper to deploy an immutable release
   or RU development-preview image.
...
| Malicious Checks metrics               | False status, data disclosure, or DoS   | Fixed server-owned PromQL, authenticated API, allowlisted normalized fields, identifier/value validation, conflict-to-unknown behavior, bounded response/sample/query/cardinality limits, and no raw upstream errors or SQLite sample persistence                                                                                        | Alert Hub cannot prove that a declared Source is independent or recognize every secret hidden in a plausible label. Restrict metric producers, scrape targets, and Prometheus access.                                |
...
| Malicious dev frontend                 | Production-data access as a tester      | The dev workflow builds only the web image on GitHub-hosted infrastructure; RU receives an immutable digest through a no-argument root wrapper. The preview mounts no SQLite, secrets, or Docker socket, requires exact API compatibility, and remains loopback-only until a separately reviewed proxy change.                           | Dev JavaScript still receives any permissions a tester grants by signing in. Restrict the preview hostname to trusted testers and protect `dev`; use an isolated API/database instead if that trust is unacceptable. |
...
| Duplicate notification under partition | User fatigue/action confusion           | Deterministic rendezvous ownership and delivery IDs use the logical incident event key; only rank zero may call a provider, reserves retain durable work without timeout promotion, explicit node eligibility stabilizes incomplete inventory, and replicated attempt/result receipts map onto each node's corresponding local event row | Automatic failover is unavailable until fenced ownership exists. Ambiguous provider outcomes and conflicting topology still prevent an exactly-once claim; validate the real providers and topology.                 |
...
the short process-local cache is neither durable evidence nor permission to present an old success
as current.

Treat all executor-provided display labels as public, non-secret aliases. In particular,
`check_id`, Check name, Source, Target, Scenario, Variant, and Canary must not contain URLs, IP
addresses, account or tenant names, protocol UUIDs, tokens, credentials, subscription material, or
response content. Backend length/character validation and the field allowlist reduce accidental
exposure; it rejects embedded IP/UUID forms, reserved routes, and obvious credential markers, but
cannot identify every secret embedded in a plausible name. Restrict who may publish the metric
families and review recording rules. The UI must use normal React escaping; route values and the
server-built Grafana `var-check_id` query parameter are URL-encoded. Per-Check result/canary limits
and a 2 MiB encoded API-response ceiling fail closed with `checks_limit_exceeded` before an
